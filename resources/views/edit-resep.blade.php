...
@section('title', 'Edit Resep - Racikara')

@section('content')

@php
    $topbarBack = route('recipes.show', $recipe->id);
    $topbarBackLabel = 'Batal';
    $topbarTitle = 'Edit Resep';
    $topbarActions = '';
@endphp
...
<main style="max-width: 1100px; margin: 0 auto; padding: 28px 24px;">
    ...
    <form action="{{ route('recipes.update', $recipe->id) }}" method="POST" enctype="multipart/form-data" id="uploadForm">
        @csrf
        @method('PUT')
        <div class="upload-layout">
            ...
            <div>
                ...
                <div class="upload-form-card" style="margin-bottom:20px;">
                    <h2>📸 Foto Makanan</h2>
                    <div class="photo-upload-zone" id="uploadZone" style="{{ $recipe->image && $recipe->image !== 'placeholder-food.jpg' ? 'display:none;' : '' }}">
                        <input type="file" name="image" accept="image/png,image/jpg,image/jpeg,image/webp" id="photoInput">
                        <span class="photo-upload-icon">📷</span>
                        <div class="photo-upload-text">Klik atau seret foto makanan baru ke sini</div>
                        <div class="photo-upload-hint">Biarkan kosong jika tidak ingin mengubah foto</div>
                    </div>
                    <div class="photo-preview" id="photoPreview" style="{{ $recipe->image && $recipe->image !== 'placeholder-food.jpg' ? 'display:block;' : '' }}">
                        <img id="previewImg" src="{{ asset('assets/img/' . $recipe->image) }}" alt="Preview">
                ...
                <div class="upload-form-card" style="margin-bottom:20px;">
                    ...
                    <div class="ingredients-builder" id="ingredientsBuilder">
                        @if(old('ingredients', $ingredients))
                            @foreach(old('ingredients', $ingredients) as $ingredient)
                            <div class="ingredient-row">
                                <input type="text" name="ingredients[]" value="{{ $ingredient }}" placeholder="Contoh: 500 gr dada ayam" class="ingredient-input">
                ...
                <div class="upload-form-card" style="margin-bottom:20px;">
                    ...
                    <div class="steps-builder" id="stepsBuilder">
                        @if(old('steps', $steps))
                            @foreach(old('steps', $steps) as $index => $step)
                            <div class="step-row">
                                <div class="step-num-badge">{{ $index + 1 }}</div>
                                <textarea name="steps[]" placeholder="Campurkan bumbu..." rows="2" class="step-input">{{ $step }}</textarea>
                ...
                <div class="preview-card" id="previewCard">
                    ...
                    <div class="preview-recipe-img" id="previewImgWrapper">
                        @if($recipe->image && $recipe->image !== 'placeholder-food.jpg')
                            <img src="{{ asset('assets/img/' . $recipe->image) }}" alt="Preview" style="width:100%;height:180px;object-fit:cover;">
                        @else
                            <span>📷</span>
                        @endif
                    </div>
                    ...
                    <div class="preview-body">
                        <div class="preview-title" id="previewTitle">{{ old('title', $recipe->title) }}</div>
                        <div class="preview-desc" id="previewDesc">{{ old('description', $recipe->description) }}</div>

                        <div class="preview-info">
                            <div class="preview-info-row">
                                <span class="label">🗂️ Kategori</span>
                                <span class="value" id="previewCat">{{ $recipe->category->name }}</span>
                            </div>
                            <div class="preview-info-row">
                                <span class="label">⏱️ Waktu</span>
                                <span class="value" id="previewTime">{{ old('cooking_time', $recipe->cooking_time) }} menit</span>
                            </div>
                            <div class="preview-info-row">
                                <span class="label">📊 Kesulitan</span>
                                <span class="value" id="previewDiff">{{ old('difficulty', $recipe->difficulty) }}</span>
                            </div>
                            <div class="preview-info-row">
                                <span class="label">🍽️ Porsi</span>
                                <span class="value" id="previewPortion">{{ old('portion', $recipe->portion) }} porsi</span>
                            </div>
                            <div class="preview-info-row" id="previewCalRow" style="{{ old('calories', $recipe->calories) ? 'display:flex;' : 'display:none;' }}">
                                <span class="label">🔥 Kalori</span>
                                <span class="value" id="previewCal">{{ old('calories', $recipe->calories) }} kkal</span>
                            </div>
                        </div>
...
// Initial char count
updateCharCount(document.getElementById('descInput'), 'descCount', 200);
